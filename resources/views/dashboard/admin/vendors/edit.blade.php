                ...
                @component('dashboard.layouts.partials.card')
                    ...
                    @slot('cardHeader')
                        @include('dashboard.layouts.partials.create-new-button', [
                            'link' => route('dashboard.admin.vendors.users.create', $item->id)
                        ])
                    @endslot
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @include('dashboard.layouts.partials.button-list-delete-big', [
                                            'link' => route('dashboard.admin.vendors.users.delete', ['id' => $item->id, 'uid' => $user->id])
                                        ])
                                    </td>
                                </tr>
                            @endforeach
            ...
            @if(isset($sites))
                <div class="col-md-12">
                    @component('dashboard.layouts.partials.card')
                        ...
                        @slot('cardHeader')
                            @include('dashboard.layouts.partials.create-new-button', [
                                'link' => route('dashboard.admin.vendors.sites.create', $item->id)
                            ])
                        @endslot
                            ...
                            <table class="table table-hover table-condensed no-footer" role="grid">
                                ...
                                @foreach($sites as $site)
                                    ...
                                    <td>
                                        @include('dashboard.layouts.partials.button-list-delete-big', [
                                            'name'  => 'Удалить',
                                            'link' => route('dashboard.admin.vendors.sites.delete', ['id' => $item->id, 'siteId' => $site->id])
                                        ])
                                        @include('dashboard.layouts.partials.button-list-edit-big', [
                                            'name'  => 'Запросы',
                                            'link' => route('dashboard.admin.vendors.sites.requests', ['id' => $item->id, 'siteId' => $site->id])
                                        ])
                                    </td>
                                    </tr>
                                @endforeach
